...
while (1) {
	if (read_temperature)
	// Read Temperature with DHT11 // Set LEDs
	if (read_distance)
	// Print Measurements
	if (clear_LCD)
	// Clear and power off LCD
}
```
...
The video demonstration of the functionality of our system is uploaded at https://photos.app.goo.gl/3Umfx6LkWDjmgwYQ8. In the video the system is set to light the red, green and blue LEDs when the temperature reaches 27, 30, 23 degrees respectively, while the screen lights up when the sensor measures a distance of less than or equal to 8cm. The video shows the message displayed on the screen after 2 minutes, the turning on of the red LED and the function of the distance sensor. The video also demonstrates that the screen turns off 10 seconds after a message is displayed, while if another message appears during the display of one message, the time starts counting from the beginning.